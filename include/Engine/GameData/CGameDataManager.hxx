...
public:
	...
	static CGameSettings *get_game_settings(void);
	...
	static T get_setting_intransient(std::string s)
	{
		...
		typename std::unordered_map<std::string, T>::iterator i = valMap.find(s);
		if(i != valMap.end())
		{
			return i->second;
		}

		CGameSettings *settings = CGameDataManager::get_game_settings();
		T k;
 ...
 */
template<>
Archetype const * CGameDataManager::get_arch<Archetype>(ArchId const uiArchId);